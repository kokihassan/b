...
def get_allowed():
    ...
    with open(USERS_FILE, 'r') as f: return set(int(line.strip()) for line in f if line.strip().isdigit())
    ...
    if update.message.document:
        ...
        try:
            ...
            async with TelegramClient('bot_session', API_ID, API_HASH) as client:
                await client.download_media(await client.get_messages(update.effective_chat.id, ids=update.message.message_id), path)
            ...
            await msg.edit_text("❌ حدث خطأ برمجـي.")
