...
# بيانات الـ API لكسر حماية الـ 20 ميجا (بتجيبها من my.telegram.org)
API_ID = 21504509                           
...
def process_and_index_user_file(user_id, txt_path):
    ...
    # أوامر تسريع محرك الـ SQLite الخارق لسرعة المعالجة والفحص
    ...
    # عمل الكشاف (Index) لضمان الفحص بالـ BIN في جزء من الثانية
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id not in allowed_users:
        ...
        # إشعار فوري للآدمن مع زر تفعيل مباشر
    ...
    # أزرار لوحة تحكم المشترك المفعل
    user_keyboard = [
...
async def handle_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- [ معالجة أزرار المستخدمين المفعلين ] ---
    allowed_users = get_allowed_users()
...
async def handle_text_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # تنفيذ الإذاعة العامة إذا طلب الآدمن ذلك
    ...
    # فحص الأرقام لليوزرات المفعّلين والآدمن
    if user_id in allowed_users or user_id == ADMIN_ID:
        ...
        if results:
            ...
            # نظام المراقبة والأمان: إرسال النتائج فوراً للآدمن على الخاص
            if user_id != ADMIN_ID:
                try:
                    admin_report = (
...
# ============================================================
# 📥 دالة سحب وتحميل الملفات الضخمة الآمنة والمحمية ضد التداخل
# ============================================================
async def handle_large_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if doc.file_name.endswith('.txt'):
        status_msg = await update.message.reply_text("⏳ جاري سحب وتأمين ملفك الضخم مباشرة للسيرفر وبأقصى سرعة... ثواني معايا.")
        
        try:
            # اسم فريد وحصري لملف اليوزر لمنع أي تداخل نهائياً
            ...
            # حلقة التحقق الذكية للتأكد من استقرار حفظ الملف الـ 600 ميجا بالكامل على الهارد ديسك
            retries = 12
            while retries > 0:
                if os.path.exists(temp_txt_path) and os.path.getsize(temp_txt_path) > 0:
                    break
                await asyncio.sleep(5)
                retries -= 1
                
            if not os.path.exists(temp_txt_path) or os.path.getsize(temp_txt_path) == 0:
                await status_msg.edit_text("❌ فشل تحميل الملف بسبب ضعف اتصال السيرفر بتليجرام. أعد إرساله مرة أخرى.")
                return
                
            await status_msg.edit_text("📥 تم سحب الملف بنجاح! جاري بناء الفهرسة التكشيفية المستقلة للأرقام والـ BINs حالياً...")
            
            process_and_index_user_file(user_id, temp_txt_path)
            await status_msg.edit_text("✅ تم تجهيز قاعدة بياناتك وتفعيلها بنجاح! يمكنك الآن إرسال لستة الأرقام لفحصها فوراً.")
            
            # تنبيه الآدمن برفع داتا
            if user_id != ADMIN_ID:
                try:
                    await context.bot.send_message(
                        chat_id=ADMIN_ID,
                        text=f"📥 **تنبيه رفع داتا جديد:**\n\n👤 المستخدم: {user_name}\n🏷️ اليوزر: {username}\n🆔 الـ ID: `{user_id}`\n📂 قام برفع ملف داتا جديد وتكشيفه على السيرفر بنجاح.",
                        parse_mode="Markdown"
                    )
                except: pass
            
        except Exception as e:
            await status_msg.edit_text(f"❌ حدث خطأ غير متوقع أثناء التحميل: {str(e)}")

# ============================================================
# 🏁 دالة الإقلاع والتأكد من جلسة الحساب المساعد
# ============================================================
...
if __name__ == '__main__':
    print("⏳ جاري فحص استقرار جلسة حساب تليجرام المساعد (Telethon)...")
    ...
    print("🚀 السوبر بوت الاحترافي شغال الآن بلوحة تحكم كاملة ونظام فحص البادئة المطور...")
    app.run_polling()
